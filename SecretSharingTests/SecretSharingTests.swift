...
//                ],
        ...
        for test in testVectors {
            ...
            let secret = data(fromHex: hexsecret)
            
            let ssss:SecretSharing
            
            switch secret.count {
            case 128/8:
                ssss = SecretSharing(version: .compact128)
            case 104/8:
                ssss = SecretSharing(version: .compact104)
            case 96/8:
                ssss = SecretSharing(version: .compact96)
            default:
            ...
            for (key, definedShares) in test where key != "secret" {
                ...
                let shares:[Data] = ssss.split(secret: secret, threshold: m, shares: n)
...
//                }
    ...
    func hex(fromData data: Data) -> String {
